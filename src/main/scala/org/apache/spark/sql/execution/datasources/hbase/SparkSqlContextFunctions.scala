	...
	def hbaseTableAsDataFrame(table: String, zkUrl: Option[String] = None,
		...
		val wrappedConf = {
			...
			if(options.contains(HBASE_TABLE_SCHEMA)){
				...
				options(HBASE_TABLE_SCHEMA).split(",", -1).map(field =>
					if(!field.startsWith(":")){
						str += field
					}
				)
...
//				log.warn("hBaseTableSchemas: " + hBaseTableSchemas.mkString(" "))
//				log.warn("sparkTableSchema: " + sparkTableSchemas.mkString(" "))
				...
				if(df_rowkey != null){
					...
					val hbaseRDD = df_rowkey.rdd.mapPartitions(p => {
						val hbasePool = HBasePool(zk(0), zk(1))
						val hBaseProxy = hbasePool.borrowObject()
						val hTable = hBaseProxy.getTable(table)
						val gets = new util.ArrayList[Get]()
						p.foreach(r => gets.add(new Get(rowkeySetter.apply(r))))
						val results = hTable.get(gets)
						hTable.close()
						hbasePool.returnObject(hBaseProxy)
						results.iterator
